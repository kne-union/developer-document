        ...
        <div className={styles.page}>
          <section className={`${styles.headerPanel} ${styles.listHeader}`}>
            <div className={styles.headerTop}>
              <div className={styles.titleBlock}>
                <div className={styles['headerIdentity']}>
                  ...
                  <Text className={styles['headerIdentityText']}>{formatMessage({ id: 'blog.list.identityLabel' })}</Text>
                </div>
                <Title level={2} className={styles.pageTitle}>
                  {formatMessage({ id: 'blog.list.pageTitle' })}
                </Title>
                <Paragraph className={styles.pageDescription}>{formatMessage({ id: 'blog.list.pageDescription' })}</Paragraph>
              </div>
              <div className={styles.searchBox}>
                <Search
                  ...
                  size="large"
                  placeholder={formatMessage({ id: 'blog.list.searchPlaceholder' })}
                  value={keyword}
                  onChange={event => {
            ...
            <div className={styles.filterRow}>
              <Text className={styles.filterLabel}>{formatMessage({ id: 'common.categoryLabel' })}</Text>
              <Fetch
                {...apis.group.groupList}
                params={{ type: 'blog', output: 'list' }}
                render={({ data, loading }) => {
                  if (loading) {
                    return <Text className={styles.filterLabel}>{formatMessage({ id: 'common.loading' })}</Text>;
                  }
            ...
            <div className={styles.filterRow}>
              ...
              <Space wrap className={styles.filterList} align="center">
                ...
                <RangePicker
                  ...
                  size="small"
                  placeholder={[formatMessage({ id: 'blog.list.publishTimeStart' }), formatMessage({ id: 'blog.list.publishTimeEnd' })]}
              ...
              if (list.length === 0) {
                return (
                  <div className={styles.emptyState}>
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={formatMessage({ id: 'blog.list.emptyDescription' })} />
                  </div>
                );
              }
                ...
                <>
                  <div className={styles.summaryBar}>
                    <div className={styles.summaryItem}>
                      <Text className={styles.summaryLabel}>{formatMessage({ id: 'blog.list.articleCount' })}</Text>
                      <Text className={styles.summaryValue}>{total}</Text>
                    </div>
                    <div className={styles.summaryItem}>
                      <Text className={styles.summaryLabel}>{formatMessage({ id: 'blog.list.browseMode' })}</Text>
                      <Text className={styles.summaryValue}>{isLoggedIn ? formatMessage({ id: 'blog.list.loggedIn' }) : formatMessage({ id: 'blog.list.publicMode' })}</Text>
                    </div>
                  </div>
                  ...
                  {total > pageSize && (
                    <div className={styles.paginationPanel}>
                      <Pagination
                        ...
                        showTotal={value => formatMessage({ id: 'blog.list.totalArticles' }, { value })}
                      />
                    </div>
                  )}
                </>
